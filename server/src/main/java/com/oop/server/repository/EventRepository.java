...
public interface EventRepository extends CrudRepository<EventModel, String>{
    ...
    @Query(value = "select * from events as e inner join users as u on u.user_id = e.user_id where e.date like %:date% and u.role = 'ADMIN'", nativeQuery = true)
    List<EventModel> findAdminEvent();
