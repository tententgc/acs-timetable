...
public interface EventRepository extends CrudRepository<EventModel, String>{
    ...
    @Query(value = "select * from events as e inner join users as u on u.user_id = e.user_id where e.event_date like %:date% and (u.user_id = :user_id or u.role = 'ADMIN') order by hex_code", nativeQuery = true)
    List<EventModel> findByDate(@Param("date") String date, @Param("user_id") String user_id);

    @Query(value = "select * from events as e inner join users as u on u.user_id = e.user_id where e.event_date like %:date% and u.role = 'ADMIN' order by hex_code", nativeQuery = true)
    ...
    @Query(value = "insert into events(event_id, header, description, event_date, time_range, create_at, update_at, user_id, hex_code) values(:event_id, :header, :description, :event_date, :time_range, :create_at, :update_at, :user_id, :hex_code)", nativeQuery = true)
